<?php

/**
 * SAML 2.0 remote IdP metadata for SimpleSAMLphp.
 *
 * Remember to remove the IdPs you don't use from this file.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-idp-remote
 */
/*
$metadata['https://idp.du.com/simplesaml/module.php/saml/sp/metadata.php/idp.du.com'] = [
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://idp.du.com/simplesaml/module.php/saml/sp/saml2-logout.php/idp.du.com',
        ],
    ],
    'AssertionConsumerService' => [
        [
            'index' => 0,
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST',
            'Location' => 'https://idp.du.com/simplesaml/module.php/saml/sp/saml2-acs.php/idp.du.com',
        ],
        [
            'index' => 1,
            'Binding' => 'urn:oasis:names:tc:SAML:1.0:profiles:browser-post',
            'Location' => 'https://idp.du.com/simplesaml/module.php/saml/sp/saml1-acs.php/idp.du.com',
        ],
        [
            'index' => 2,
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Artifact',
            'Location' => 'https://idp.du.com/simplesaml/module.php/saml/sp/saml2-acs.php/idp.du.com',
        ],
        [
            'index' => 3,
            'Binding' => 'urn:oasis:names:tc:SAML:1.0:profiles:artifact-01',
            'Location' => 'https://idp.du.com/simplesaml/module.php/saml/sp/saml1-acs.php/idp.du.com/artifact',
        ],
    ],
    'contacts' => [
        [
            'emailAddress' => 'mesbah.csesust@gmail.com',
            'contactType' => 'technical',
            'givenName' => 'Mesbah',
        ],
    ],
];
*/
$metadata['https://idp.du.com/simplesaml/saml2/idp/metadata.php'] = [
    'metadata-set' => 'saml20-idp-remote',
    'entityid' => 'https://idp.du.com/simplesaml/saml2/idp/metadata.php',
    'SingleSignOnService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://idp.du.com/simplesaml/saml2/idp/SSOService.php',
        ],
    ],
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://idp.du.com/simplesaml/saml2/idp/SingleLogoutService.php',
        ],
    ],
    'certData' => '',
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:transient',
    'contacts' => [
        [
            'emailAddress' => 'mesbah.csesust@gmail.com',
            'contactType' => 'technical',
            'givenName' => 'Mesbah',
        ],
    ],
];
